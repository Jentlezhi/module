...
@interface CYTBasicViewController()<UIScrollViewDelegate,UITableViewDataSource,UITableViewDelegate>
/** 导航栏图片 */
...
/** 当前页面的表格视图 */
@property(weak, nonatomic) UITableView *configTableView;
/** 当前页面的表格视图间距 */
@property(assign, nonatomic) UIEdgeInsets configTableViewEageInsets;
/** 上次选中的索引 */
...
- (UIImageView *)navigationZone{
    if (!_navigationZone) {
        ...
        navigationZone.frame = CGRectMake(0, 0, kScreenWidth, CYTStatusBarHeight + CYTNavigationBarHeight);
...
- (UILabel *)navTitleLabel{
    if (!_navTitleLabel) {
        ...
        _navTitleLabel.lineBreakMode = NSLineBreakByTruncatingMiddle;
        CGFloat navTitleLabelX = kScreenWidth*0.2;
        CGFloat navTitleLabelY = 0;
        CGFloat navTitleLabelW = kScreenWidth*0.6;
        ...
        [self.navigationBar addSubview:_navTitleLabel];
    }
...
- (CYTNoDataView *)noDataView{
    if (!_noDataView) {
        ...
        _noDataView.content = self.noDataViewText;
        _noDataView.imageName = @"bg_information_hl";
    }
    return _noDataView;
}
/**
 ...
 *  状态栏颜色
 */
- (void)setStatusBarStyle:(CYTStatusBarStyle)statusBarStyle{
    _statusBarStyle = statusBarStyle;
    if (statusBarStyle == CYTStatusBarStyleDefault) {
...
- (void)baseBasicConfig{
    ...
    [[[NSNotificationCenter defaultCenter] rac_addObserverForName:CYTTabBarDidSelectNotification object:nil] subscribeNext:^(id x) {
        if (weakSelf.lastSelectedIndex == weakSelf.tabBarController.selectedIndex && [weakSelf.view isVisibleOnKeyWindow]) {
            !weakSelf.doubleClickTabBarBlock?:weakSelf.doubleClickTabBarBlock();
        }
        weakSelf.lastSelectedIndex = weakSelf.tabBarController.selectedIndex;
    }];
}
/**
 *  初始化子控件
...
- (void)setNavigationBarColor:(UIColor *)navigationBarColor{
    ...
    _statusBar.backgroundColor = navigationBarColor;
    _dividerLine.backgroundColor = navigationBarColor;
}
/**
 * hiddenNavigationBarLine的setter方法
 */
- (void)setHiddenNavigationBarLine:(BOOL)hiddenNavigationBarLine{
    _hiddenNavigationBarLine = hiddenNavigationBarLine;
...
- (void)setNavTitle:(NSString *)navTitle{
    _navTitle = navTitle;
    navTitle = navTitle.length?navTitle:@"";
...
- (void)setBackBtnColor:(UIColor *)backBtnColor{
    ...
    UIImage *btnImage = [[UIImage imageNamed:@"nav_back"] drawImageWithColor:backBtnColor];
...
- (void)createNavBarWithTitle:(NSString *)tittle andShowBackButton:(BOOL)isShowBackButton showRightButtonWithTitle:(NSString *)rightButtonTitle{
    ...
    if (rightButtonTitle.length != 0) {
        ...
        CGFloat rightButtonMaxW = kScreenWidth*0.2;
        CGFloat rightButtonTitleW = [rightButtonTitle sizeWithFont:self.rightItemButton.titleLabel.font maxSize:CGSizeMake(CGFLOAT_MAX, rightButtonH)].width;
        CGFloat rightButtonW = rightButtonTitleW>rightButtonMaxW?rightButtonMaxW:rightButtonTitleW;
...
- (void)setRightButtonTitle:(NSString *)rightButtonTitle{
    ...
    if (!rightButtonTitle || !rightButtonTitle.length) {
        rightButtonTitle = @"";
    }
...
- (void)hideNavigationBarWithAnimation:(BOOL)animation{
    ...
    originF.origin.y=-CYTViewOriginY;
    CGRect currentF = originF;
    if (animation) {
        [UIView animateWithDuration:0.35f animations:^{
            weakSelf.navigationZone.frame = currentF;
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        for (UIView *itemView in self.view.subviews) {
            if ([itemView isKindOfClass:[UITableView class]]) {
                ...
                self.configTableView.showsVerticalScrollIndicator = NO;
                self.configTableViewEageInsets = tableView.contentInset;
                UIEdgeInsets tempInsets = tableView.contentInset;
                tempInsets.bottom = 0;
                tableView.contentInset = tempInsets;
            }
        }
        if (tableView) {
...
- (void)dismissNoNetworkView{
    if (self.noNetworkView) {
        self.configTableView.showsVerticalScrollIndicator = YES;
        self.configTableView.contentInset = self.configTableViewEageInsets;
        self.configTableView = nil;
        [UIView animateWithDuration:0.2f animations:^{
            self.noNetworkView.alpha = 0.1f;
...
- (void)showNoDataView{
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        //布局子控件
        [self.noDataView.noDataImageView makeConstraints:^(MASConstraintMaker *make) {
            make.centerY.equalTo(self.noDataView).offset(-CYTAutoLayoutV(140.f));
            make.centerX.equalTo(self.noDataView);
        }];
        
    });
}
- (NSString *)noDataViewText{
    if (!_noDataViewText.length) {
        return @"- 暂时没有您想要的数据 -";
    } else {
        return _noDataViewText;
    }
}
/**
 *  移除无数据界面
...
- (void)dismissNoDataView{
    if (self.noDataView) {
        self.configTableView.delegate = nil;
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView{
    ...
    if (offsetY > 0) {
        [self.configTableView setContentOffset:CGPointMake(0, 0) animated:NO];
    }
}
#pragma mark - 懒加载
...
- (UITableView *)basicTableView{
    if (!_basicTableView) {
        _basicTableView = [[UITableView alloc] initWithFrame:CGRectZero style:UITableViewStyleGrouped];
        if (@available(iOS 11.0, *)) {
            _basicTableView.contentInsetAdjustmentBehavior = UIScrollViewContentInsetAdjustmentNever;
            _basicTableView.estimatedSectionFooterHeight = 0;
            _basicTableView.estimatedSectionHeaderHeight = 0;
        }
        _basicTableView.backgroundColor = CYTLightGrayColor;
        _basicTableView.delegate = self;
        _basicTableView.dataSource = self;
        _basicTableView.tableFooterView = [[UIView alloc] init];
        _basicTableView.separatorStyle = UITableViewCellSeparatorStyleNone;
        _basicTableView.estimatedRowHeight = CYTAutoLayoutV(100);
